...
/* The database connection class can be stored outside of the example files so you cannot
   see the connection information. Also it is more secure */
...
class Pdo_methods extends Db_conn
{
    ...
    /* This method is for all SELECT statements that need to have a binding to protect the data.
       The script takes the SQL statements and the binding array as its parameters and performs
       the query. It will run the query and return the result as an associative array or an
       error string. */
    ...
    /* This function does the same as the above but does not need any binded parameters
       as no parameters are passed */
    ...
    /* Because only SELECT queries return a value, this method does all the rest:
